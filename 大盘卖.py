...
method_name = [{
                ...
                # 'SVC(linear, C=1.5)':svm.SVC(kernel='linear',probability=True),
...
stocklist=['SPY'] #'AMZN','GOOG','FB','JNJ','V','PG','JPM','UNH','MA','INTC','VZ','HD','T','PFE','MRK','PEP']
for stock in stocklist:
    ...
    selljudge(df,loss=0.009,cycle=10)
    df.dropna(axis=0, how='any', inplace=True)#Get rid of rows with NA value
    #Retrive X and y 
    X=df.loc[:,['# Inter 10-day','Intersection','MAVOL200','MAVOL20','MAVOL10','MAVOL5','SP500_ROC','VIX_ROC','Close_ROC','rsv','K','D','J',
...
for threshold in np.arange(0.75,0.81,0.015):
    dfplot['Sell']=0
    dfplot['SellPrice']=0
    ...
    plt.figtext(0.35,0.1,'Sell Ratio='+str(Sellratio)+'%' , fontsize=13)
    plt.legend(loc='upper left')
    plt.show()
 #%%  Visualize the points       
clfsell =GaussianNB(var_smoothing=1e-02)
clfsell.fit(xtrain, ytrain)
sellpredicted = clfsell.predict_proba(xtest)    
dfplot=pd.DataFrame()
dfplot.loc[:,'Close']=df[3500:]['Close']
